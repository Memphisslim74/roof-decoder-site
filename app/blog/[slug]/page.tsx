...
export default async function BlogPostPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const post = postBySlug(slug);
  if (!post) notFound();
  const publishedLabel = new Intl.DateTimeFormat("en-US", { dateStyle: "long", timeZone: "UTC" }).format(new Date(`${post.published}T00:00:00Z`));
  ...
  return (
    <PageShell>
      ...
      <article>
        <header className="article-header"><div className="container article-header-inner"><nav className="breadcrumbs" aria-label="Breadcrumb"><Link href="/">Home</Link><span>›</span><Link href="/blog">Articles</Link><span>›</span><span>{post.category}</span></nav><p className="eyebrow">{post.category}</p><h1>{post.title}</h1><p className="article-deck">{post.description}</p><div className="article-byline"><span>By My Family Roofer</span><span>Published {publishedLabel}</span><span>{post.readTime}</span></div></div></header>
        <div className="container article-image"><img src={post.image} alt={post.imageAlt} /></div>
        <div className="section container content-with-aside article-layout"><div className="article-body"><p className="article-intro">{post.intro}</p>{post.sections.map((section) => <section key={section.heading}><h2>{section.heading}</h2>{section.paragraphs?.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{section.bullets && <ul>{section.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul>}{section.links && <ul>{section.links.map((link) => <li key={link.href}><Link href={link.href}>{link.label}</Link></li>)}</ul>}{section.subsections?.map((subsection) => <div key={subsection.heading}><h3>{subsection.heading}</h3>{subsection.paragraphs?.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}{subsection.bullets && <ul>{subsection.bullets.map((bullet) => <li key={bullet}>{bullet}</li>)}</ul>}{subsection.links && <ul>{subsection.links.map((link) => <li key={link.href}><Link href={link.href}>{link.label}</Link></li>)}</ul>}</div>)}</section>)}<div className="article-note"><strong>Local focus:</strong> This guide is written for homeowners in Berthoud, Loveland, Fort Collins, Greeley, Boulder, and nearby Northern Colorado communities. Property, code, product, contract, and policy details vary.</div></div><div className="sticky-aside"><InlineLead context={post.category === "Hail Damage" ? "hail concern" : "roofing question"} /><nav className="article-toc"><strong>In this guide</strong>{post.sections.map((section) => <span key={section.heading}>{section.heading}</span>)}</nav></div></div>
      ...
      <div className="container cta-wrap"><LeadCta title="Want a Local Answer for Your Roof?" text="Talk with My Family Roofer about your estimate, hail concerns, or next step in Berthoud, Loveland, Fort Collins, Greeley, Boulder, or a nearby community." /></div>
    </PageShell>
  );
}
